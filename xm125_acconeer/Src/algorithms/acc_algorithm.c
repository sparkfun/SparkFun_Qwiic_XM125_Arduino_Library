...
void acc_algorithm_roll_and_push_f32_complex_matrix(float complex *data, uint16_t rows, uint16_t cols, const float complex *column,
                                                    bool pos_shift)
{
	if (pos_shift)
	{
...
void acc_algorithm_apply_filter(const float *a, const float *filt_data, uint16_t filt_rows, uint16_t filt_cols, const float *b,
...
void acc_algorithm_apply_filter_complex(const float *a, const float complex *filt_data, uint16_t filt_rows, uint16_t filt_cols,
...
void acc_algorithm_rfft_matrix(const float *data, uint16_t rows, uint16_t cols, uint16_t length_shift, float complex *output, uint16_t axis)
{
	...
	if (axis == 1U)
	{
		...
		for (uint16_t i = 0U; i < rows; i++)
		{
			acc_algorithm_rfft(&data[i * cols], cols, length_shift, &output[i * output_cols], 1U);
		}
	}
	else if (axis == 0U)
	{
		...
		{
			acc_algorithm_rfft(&data[i], rows, length_shift, &output[i], cols);
		}
	}
	else
	{
...
{
	small_fft(data, data_length, length_shift, output, stride);
}
...
void acc_algorithm_fft_matrix(const float complex *data, uint16_t rows, uint16_t cols, uint16_t length_shift, float complex *output, uint16_t axis)
{
	...
	if (axis == 1U)
	{
		...
		for (uint16_t i = 0U; i < rows; i++)
		{
			acc_algorithm_fft(&data[i * cols], cols, length_shift, &output[i * output_cols], 1U);
		}
	}
	else if (axis == 0U)
	{
		for (uint16_t i = 0U; i < cols; i++)
		{
			acc_algorithm_fft(&data[i], rows, length_shift, &output[i], cols);
		}
	}
	else
	{
...
void acc_algorithm_fftshift_matrix(float *data, uint16_t rows, uint16_t cols)
{
	for (uint16_t i = 0U; i < cols; i++)
	{
		acc_algorithm_fftshift(&(data[i]), rows, cols);
	}
}
                                ...
                                float               fs)
{
	for (uint16_t i = 0U; i < cols; i++)
	{
		acc_algorithm_welch(&(data[i]), rows, segment_length, data_buffer, fft_out, &(psds[i]), window, length_shift, fs,
		                    cols);
	}
}
                         ...
                         uint16_t            stride)
{
	...
	for (uint16_t i = 0U; i < num_segments; i++)
	{
		...
		acc_algorithm_fft(data_buffer, segment_length, length_shift, fft_out, 1U);
...
float acc_algorithm_calculate_cfar(const float *data,
                                   ...
                                   uint16_t    idx)
{
	uint16_t margin                        = cfar_win_length + cfar_guard_length;
	...
	if (idx <= margin)
	{
		for (uint16_t j = 0U; j < cfar_win_length; j++)
		{
			sum += data[j];
		}
	}
	...
	return ((sum / (float)cfar_win_length) + min) / cfar_sensitivity;
}
